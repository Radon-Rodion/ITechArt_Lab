...
export default webpackMockServer.add((app, helper) => {
  ...
  app.post("/api/auth/signIn", (req, res) => {
    const response = findUserInfo(req.body.login);
  ...
  app.put("/api/auth/signUp", (req, res) => {
    const existingInfo = findUserInfo(req.body.login);
    if (existingInfo === undefined) res.json({ body: req.body || null, success: true });
    else res.status(400).json({ body: undefined || null, success: false });
  });
});
